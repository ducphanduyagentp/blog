...
- Look carefully.
- Change default credentials. There is a potato out there died for each of the unchanged credentials.
...
0. **CHANGE DEFAULT PASSWORDS**: Just change it. If there's too much work, try to find a way to automate the process.
1. Have a working firewall script. My scripts break on all the competitions that I have ever been to.
1. The scored web application communicates with the databases. When the databases are down, we lose points for the web application as well. This means it's not about just defending separate machines, but keeping the communication between the components is also critical.
2. Monitoring file integrity is critical.
3. Check for PAM backdoor.

<a name="vietnamese-lockdownv3"></a>
